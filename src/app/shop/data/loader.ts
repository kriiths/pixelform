...
/**
 * Dynamically loads products from the local file system (public/products/{category}/{productId}/info.json)
...
async function loadProductsFromFileSystem(category: string): Promise<Product[]> {
  ...
  for (const productDir of productDirs) {
    ...
    try {
      ...
      // Check if images are stored in metadata
      let images: string[] = [];
      ...
      if (productInfo.images && Array.isArray(productInfo.images)) {
        ...
        // Fallback: Get images from the images directory
